...
### Testing
Two test layers. `npm run test:node` runs the fast node-only unit tests for the runtime-pure modules (vitest, no Electron) under `client/test/unit/`. The rest run in a real extension host (vscode-test) against the sample mod in `client/test/sample/`, picked by label from `.vscode-test.mjs`: `npm test` runs the `unit` label (VS Code API, no language server), `npm run test:smoke` adds the activation suite against the real server, and `npm run test:host` runs everything, including the hover/completion suites that need rule data the sample workspace can't resolve. CI gates on `test:node`, `test`, `test:smoke`, and `test:host`.
