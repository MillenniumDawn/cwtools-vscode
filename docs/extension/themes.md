...
The bundled themes live under `extension/package/themes/`. Each is a VS Code color theme
...
`paradox` language via `configurationDefaults` in `extension/package/package.json`.
...
1. Copy an existing manifest (for example `Paradox-Nord.tmLanguage.json`) to a
   new file under `extension/package/themes/`.
...
4. Register the theme in `extension/package/package.json` under the `themes` array
   (`label`, `uiTheme`, `path`).
