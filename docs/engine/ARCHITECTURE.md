...
The engine is a Rust workspace under `engine/` with 16 crates: a parser, a
...
- `error_codes`: the shared `CW###` catalog (on `i18n`).
...
The dependency graph is acyclic. `i18n`, `string_table`, and `game` are the
shared leaves; `error_codes` adds only `i18n`.
...
7. `localization/src/scope_validation.rs`: add the variant to `game_to_engine`'s
   pass-through list (else loc scope checks are skipped for that game, unless
   a config-driven registry is present).
...
3. Add a row to `docs/engine/ERROR_CODES.md`.
