...
The extension release bundles `cwtools-server` for each supported platform.
Build the standalone CLI from source with `cargo build --release` from
`engine/`. [The build guide](BUILD.md) covers the details.
...
The `engine/` directory is the whole engine, a Rust workspace of 16 crates under
...
The diagnostics regression guards live in the repository's [`scripts/`](../../scripts/)
directory.

## Documentation

- [Architecture](ARCHITECTURE.md) — the crate map, the batch pipeline, the CLI-vs-LSP split, and LSP features like the idle-gated background reindex.
- [CWXXX error/warning code reference](ERROR_CODES.md) — full catalog of diagnostic codes emitted by the Rust validator.
- [Math expressions](MATH_EXPRESSIONS.md) — the HOI4 math-block operators and what the validator enforces inside one.
