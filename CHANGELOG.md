...
* The one-second no-reply answer `checkCytoscapeRendered` grew in #212 has a test. It was the fix for a flake (#210) and shipped without one, and the host suite cannot cover it: a real webview replies, so the host only ever walks the happy path. A node test drives the panel against a fake webview with fake timers and pins all three behaviours, the reply, the one-second answer when none comes, and the fresh post per call the poll loop depends on. Confirmed to fail when the time bound is widened.
