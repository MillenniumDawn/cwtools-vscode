...
* `check` and `build` in `pr.yml` no longer re-download a full VS Code build on every run. `.vscode-test/` is cached via `actions/cache`, keyed per OS and job with a restore-keys fallback, since the pinned `vscode: "stable"` in `.vscode-test.mjs` means the resolved build isn't known ahead of time; `vscode-test` tops the cache up only when `stable` has actually moved. (#137)
