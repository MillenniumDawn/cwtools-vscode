...
// string itself, translated in extension/package/l10n/bundle.l10n.<locale>.json. A key
...
const packageDir = path.join(repoRoot, "extension", "package");
const l10nDir = path.join(packageDir, "l10n");
const extensionDir = path.join(repoRoot, "extension", "src", "host");
...
const english = loadJsonc(path.join(packageDir, "package.nls.json"));
...
suite("nls", () => {
	...
	for (const file of TRANSLATIONS) {
		test(`${file} has no keys the English file dropped`, () => {
			const translated = loadJsonc(path.join(packageDir, file));
...
suite("l10n", () => {
	...
	test("the extension has runtime strings to translate", () => {
		assert.ok(source.size > 0, "no l10n.t() calls found in extension/src/host");
	});
	...
	test("the manifest points at the directory the bundles are in", () => {
		...
		assert.ok(
			manifest.l10n,
			"extension/package/package.json declares no l10n directory",
		);
		const declared = path.resolve(packageDir, manifest.l10n);
		assert.strictEqual(
			declared,
			l10nDir,
