...
export async function resolveRulesCache(language: string, cacheDir: string): Promise<RulesSetup> {
		...
		async () => {
			try {
				...
				await client.sendRequest(ExecuteCommandRequest.type, { command: 'reloadrulesconfig', arguments: [] })
					.catch(err => logError('Failed to reload rules after background fetch', err));
			} catch (err) {
