...
suite('FileExplorer — filesToTreeNodes', () => {
	...
	test('nests files at multiple depths and merges siblings', () => {
		const files: FileListItem[] = [
			{ scope: 'common', uri: 'file:///common/buildings/castle.txt', logicalpath: 'buildings/castle.txt' },
			{ scope: 'common', uri: 'file:///common/buildings/temple.txt', logicalpath: 'buildings/temple.txt' },
			{ scope: 'common', uri: 'file:///common/technology.txt', logicalpath: 'technology.txt' }
		];
		const tree = filesToTreeNodes(files);
		assert.strictEqual(tree.length, 1);
		const common = tree[0];
		...
		assert.deepStrictEqual(
			buildings.children.map(c => c.fileName).sort(),
			['castle.txt', 'temple.txt']
		);
	...
	test('treats an intermediate segment without further children as a directory', () => {
		...
		const leaf: FileListItem[] = [{ scope: 's', uri: 'u1', logicalpath: 'a.txt' }];
		const nested: FileListItem[] = [{ scope: 's', uri: 'u2', logicalpath: 'dir/a.txt' }];
