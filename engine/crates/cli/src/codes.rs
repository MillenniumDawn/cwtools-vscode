    ...
    entry(s).map(|(_, c)| c.id.to_string()).ok_or_else(|| {
        format!(
            "unknown diagnostic code '{s}': expected a CWxxx code the validator emits \
             (e.g. CW100, CW113, CW225); docs/engine/ERROR_CODES.md lists them all"
        )
    })
}
...
/// `docs/engine/ERROR_CODES.md`, embedded so `explain` answers from the binary alone.
/// 30 KB of the reference the diagnostics already link to.
const REFERENCE: &str = include_str!("../../../../docs/engine/ERROR_CODES.md");
...
mod tests {
    ...
    /// Every diagnostic links to `docs/engine/ERROR_CODES.md#cw###`, so a code with no
    ...
    fn every_code_has_a_documentation_anchor() {
        let doc = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../../../docs/engine/ERROR_CODES.md"),
        )
        ...
        assert!(
            missing.is_empty(),
            "docs/engine/ERROR_CODES.md needs an <a id=\"cw###\"> anchor for: {missing:?}"
        );
    }
    ...
    fn every_code_has_a_readable_reference_row() {
        ...
        assert!(
            missing.is_empty(),
            "docs/engine/ERROR_CODES.md needs a `| <a id=…></a>CW### | Severity | Message | Meaning | \
    ...
    fn catalog_matches_the_reference() {
        for (_, code) in CATALOG {
            ...
            assert_eq!(
                ...
                "{} severity disagrees with docs/engine/ERROR_CODES.md",
                code.id
            );
            assert_eq!(
                cells[3], code.message_template,
                "{} message disagrees with docs/engine/ERROR_CODES.md",
                code.id
            );
        }
    }
    ...
    fn pending_codes_match_the_reference_status() {
        ...
        assert_eq!(
            documented, listed,
            "PENDING_CODES disagrees with the Status column in docs/engine/ERROR_CODES.md"
        );
    }
